...
export default function CategoryManager() {
  ...
  const fetchCategories = async () => {
    if (!user) return
    setLoading(true)
    // Fetch top-level categories for this user
    const data = (await getChildren(null, user.uid)) as Category[]
    setCategories(data)
    setLoading(false)
  }
  ...
  const handleAddCategory = async () => {
    ...
    await createCategory(newCategoryName, newCategoryIcon || "📂", null, user.uid)
    setNewCategoryName("")
    setNewCategoryIcon("")
    fetchCategories()
  }
    ...
    <div className="p-4 space-y-6">
      ...
      {loading ? (
        ...
        <div className="space-y-2">
          {categories.map((cat) => (
            <CategoryNode key={cat.id} category={cat} userId={user?.uid} />
          ))}
        </div>
      )}
    </div>
...
function CategoryNode({ category, userId }: { category: Category; userId?: string }) {
  ...
  const fetchChildren = async () => {
    if (!userId) return
    const subs = (await getChildren(category.id, userId)) as Category[]
    setChildren(subs)
  }
  ...
  const handleAddSub = async () => {
    if (!newSubName || !userId) return
    await createCategory(newSubName, newSubIcon || "📂", category.id, userId)
    setNewSubName("")
    setNewSubIcon("")
    fetchChildren()
    ...
    <div className="ml-4 border-l pl-4">
      ...
      {showChildren && (
        <div className="ml-6 mt-2 space-y-2">
          {children.map((child) => (
            <CategoryNode key={child.id} category={child} userId={userId} />
          ))}
          <div className="flex gap-2 flex-wrap">
            <input
              type="text"
